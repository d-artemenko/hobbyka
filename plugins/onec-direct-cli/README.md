...
```env
ONEC_HTTP_SERVICE_URL=https://server/BaseName/hs/mcp
ONEC_HTTP_USERNAME=ReadOnlyUser
ONEC_HTTP_PASSWORD=
ONEC_HTTP_TIMEOUT_MS=60000
```

The CLI also supports `.env` in the current working directory or skill directory, and an explicit file through `ONEC_HTTP_ENV_FILE`.
