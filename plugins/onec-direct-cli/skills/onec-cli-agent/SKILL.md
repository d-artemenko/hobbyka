---
name: onec-cli-agent
description: Напрямую читать метаданные и выполнять безопасные запросы на языке запросов 1С через опубликованный HTTP-сервис базы и локальный Node.js CLI. Use when Codex должен ответить по живой 1С, проверить структуру объектов или выполнить read-only выборку без OpenCode, RAG, локального MCP-сервера, SSE или Docker-прокси.
---
